...
updated_at: 2026-03-20T10:44:50Z
---
...
## Acceptance Criteria
- [ ] A `game.Player` struct exists in `internal/game/` with fields for hand, round points, cumulative points, and pass state (submitted, sent, received)
- [ ] `game.Player` exposes methods for: receiving dealt cards, submitting a pass, receiving passed cards, playing a card (removes from hand), accumulating trick points
- [ ] `runtime.playerState` no longer manages hand or points directly — delegates to an embedded or referenced `game.Player`
- [ ] `roundState` pass maps and round-points maps are replaced by per-player state on `game.Player`
- [ ] `sim` uses `game.Player` instances instead of raw `[4][]game.Card` arrays and ad-hoc point tracking
- [ ] All existing tests pass; unit tests added for `game.Player` methods
- [ ] Runtime continues to validate all player inputs (card legality, pass card count, etc.) before delegating to `game.Player`
